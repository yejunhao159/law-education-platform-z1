...
3. 案件导入页 - 100字故事化陈述
4. 当事人关系图 - Network图可视化
5. 案件时间轴 - Timeline图标注关键节点
6. 争议焦点页 - 2-3个焦点
...
### 大前提：法律规范（3-4页）
- 适用法律识别页 - 完整引用法条
- 法律要件拆解页 - Flowchart展示要件层级
- 法律解释页（可选）- 文义/体系/目的解释

### 小前提：案件事实（3-4页）
- 事实梳理页 - 5-8个关键事实
- 证据分析页 - Radar图展示三性（真实性、关联性、合法性）
- 事实与要件对应页 - 表格对照

### 结论：推理（2页）
- 推理过程页 - Flowchart展示三段论
- 争议点辨析页（可选）
...
## 图表选择
- 时间顺序 → Timeline（案件发展、学习路径）
- 多维评估 → Radar（能力提升、证据质量）
- 数量对比 → Bar（前后对比、效果对比）
- 比例构成 → Pie（时间分配、占比分析）
- 关系网络 → Network（当事人关系、法律关系）
- 流程步骤 → Flowchart（法律推理、分析流程）

## visualHints格式
\`用[图表类型]展示[内容]（[具体数值]），[辅助说明]\`

示例："用雷达图展示证据质量（真实性90%、关联性95%、合法性78%），标注评分依据"`;
  }
...
- **medium（20-25页）**：约6000 tokens - 标准完整
- **long（25-35页）**：约8000 tokens - 深度拓展
...
**直接输出Markdown文本**（302.ai自动解析）：

- \`## 页面标题\` = 一页PPT
- \`---\` = 页面分隔
- \`> 💡 设计提示：用[图表]展示[内容]（[数值]）\` = 图表可视化指令
...
### 事实分析`;
    ...
    if (teachingHighlights.visualizableData.length > 0) {
      ...
      teachingHighlights.visualizableData.forEach(chart => {
        result += `\n**${chart.title}** [${chart.type}图]：${chart.description}\n`;
      });
    }
