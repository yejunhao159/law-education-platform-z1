...
export class RuleExtractor {
  ...
  static extract(document: ProcessedDocument): ExtractedData {
    ...
    return {
      ...
      legalRelations: this.extractLegalRelations(text),
  ...
  /**
   * 提取法律关系
   * 整合自 lib/legal-analysis-engine.ts 的系统化法律关系识别
   */
  static extractLegalRelations(text: string): Array<{
    type: string
    keywords: string[]
    confidence: number
    context: string
    ...
    // 遍历所有法律关系模式
    Object.values(LEGAL_RELATION_PATTERNS).forEach(pattern => {
      ...
      pattern.keywords.forEach(keyword => {
        ...
        if (matches) {
          ...
          if (index !== -1) {
            const context = this.getContext(text, index, 80)
      ...
      // 如果匹配到关键词，计算置信度并添加到结果
      if (matchedKeywords.length > 0) {
        const confidence = Math.min(
          0.9, // 最大置信度
          (matchedKeywords.length / pattern.keywords.length) * 0.8 + 0.2
        )
