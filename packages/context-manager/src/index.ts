...
export class ContextFormatter {
  private static readonly DEFAULT_OPTIONS: FormatterOptions = {
    format: 'xml',
   ...
   */
  private static formatAsXML(data: ContextData, options: FormatterOptions): string {
    ...
    if (data.examples && data.examples.length > 0) {
      const examplesXML = data.examples.map(ex =>
   ...
   */
  private static formatXMLSection(name: string, content: string | string[], indent: string): string {
   ...
   */
  private static formatXMLObject(name: string, obj: Record<string, any>, indent: string): string {
   ...
   */
  private static formatAsJSON(data: ContextData, options: FormatterOptions): string {
   ...
   */
  private static formatAsMarkdown(data: ContextData, _options: FormatterOptions): string {
    const sections: string[] = [];
    ...
    if (data.examples && data.examples.length > 0) {
      const examplesText = data.examples.map((ex, i) =>
        `### Example ${i + 1}\n\n**Input:** ${ex.input}\n\n**Output:** ${ex.output}`
      ).join('\n\n');
      sections.push(`## Examples\n\n${examplesText}`);
    }
   ...
   */
  private static cleanData(data: ContextData): any {
    const result: any = {};
   ...
   */
  static parse(formattedContent: string, format: 'xml' | 'json' | 'markdown' = 'xml'): ContextData {
    switch (format) {
      case 'json':
   ...
   */
  private static parseXML(xml: string): ContextData {
   ...
   */
  private static parseMarkdown(markdown: string): ContextData {
    const data: ContextData = {};
   ...
   */
  static validate(data: ContextData): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
