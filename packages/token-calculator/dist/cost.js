export class CostCalculator {
    ...
    // 基于文本直接估算成本
    estimateCostFromText(inputText, outputTokens = 0, model, provider) {
    ...
    // 比较不同模型的成本
    compareCosts(inputText, outputTokens = 0, models) {
    ...
    // 获取最经济的模型建议
    getMostEconomical(inputText, outputTokens = 0, models) {
        ...
        // 找到最便宜的
        const cheapest = validComparisons.reduce((min, current) => current.cost.totalCost < min.cost.totalCost ? current : min);
        // 计算相对于最贵的节省
        const mostExpensive = validComparisons.reduce((max, current) => current.cost.totalCost > max.cost.totalCost ? current : max);
    ...
    // 计算月度成本估算
    estimateMonthlyCost(dailyInputTokens, dailyOutputTokens, model, daysPerMonth = 30) {
        const dailyUsage = {
    ...
    // 批量成本估算
    batchEstimateCost(texts, outputTokensPerText = 0, model, provider) {
    ...
    // 成本预算检查
    checkBudget(inputText, outputTokens, model, budget, provider) {
