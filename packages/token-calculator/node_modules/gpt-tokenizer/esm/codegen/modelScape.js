...
const formatNumber = function formatNumber(/** @type {number} */ value) {
    if (typeof value !== 'number')
        return `'${value}'`; // For string-based rate limits like "5 img/min"
...
const codegen = function codegen(models, snapshots, otherModels = null) {
    ...
    for (const [name, priceData] of priceMap.entries()) {
        ...
        const modelUsingThisAsCurrent = Object.values(models).find((m) => m?.default?.current_snapshot === name);
    ...
    for (const modelPath of sortedModelPaths) {
        ...
        const configString = `const ${modelVarName}_config = ${renderValue(configData)} as const satisfies ModelConfig`;
        ...
        for (const snapshotName of snapshotNames) {
            ...
            if (snapshotData && !generatedSpecs.has(snapshotName)) {
                ...
                const specString = `const ${snapshotVarName}_spec = ${renderValue(snapshotData)} as const satisfies ModelSpec`;
                output.push(specString, `export {${snapshotVarName}_spec as '${snapshotName}'}`, '');
                generatedSpecs.add(snapshotName);
            }
        }
        const { current_snapshot: currentSnapshotName, name: modelName } = configData;
        if (currentSnapshotName &&
            ...
            output.push(`// alias:`, `export { ${currentSnapshotVarName}_spec as '${modelName}' };`, '');
        }
        output.push('');
    }
    // --- Final Step: Report on missing models ---
    const missingModels = [...seenInPricing].filter((name) => !generatedSpecs.has(name) && !models[`./models-data/${name}.yaml`]);
    if (missingModels.length > 0) {
        output.push('/*', ' --- Missing Models ---', ' The following models were found in the pricing data but not in the main models/snapshots sources:');
...
//# sourceMappingURL=modelScape.js.map