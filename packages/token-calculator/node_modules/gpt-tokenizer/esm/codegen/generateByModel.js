...
await Promise.all(Object.entries(models).map(async ([_modelName, modelData]) => {
    ...
    const content = isChatModel
        ? template
            .replace(`getEncodingApi('cl100k_base', () => bpeRanks)`, `getEncodingApiForModel('${modelName}', () => bpeRanks, ${devalue.uneval(modelData)})`)
